...
$(document).ready(function () {
    ...
    getAllCategory(currentPage, pageSize).then(response => {
...
function createCategoryDOM(response) {
    ...
    for (let i = 0; i < response["lstAuditLog"].length; i++) {
        ...
        for (let j = 0; j < 6; j++) {
            if (j === 0) {
                ...
                const checkBox = $("<input>").attr({
                    type: "checkbox",
                    "data-id": response['lstAuditLog'][i]['id']
                }).addClass("form-check-input border-dark");
                ...
                if (keys[j] === "isError") {
                    ...
                    div.addClass(`badge ${response["lstAuditLog"][i][keys[j]] ? 'bg-soft-danger' : 'bg-soft-success'} rounded px-3 py-1`)
                        .text(`${response["lstAuditLog"][i][keys[j]] ? 'Error' : 'Info'}`);
                    td.append(div);
                } else {
                    td.text(response["lstAuditLog"][i][keys[j]]);
                }
                tableBodyRow.append(td);
            }
        }
...
function paginationInit(response) {
    ...
    if (currentPage <= 3) {
        first = 0
        last = totalPages < 8 ? totalPages + 1 : 8
    } else if (currentPage >= totalPages - 3) {
        first = totalPages - 4 > 0 ? totalPages - 4 : 0
        last = totalPages + 1;
    } else {
        first = currentPage - 4
        last = currentPage + 4
    ...
    for (let i = first; i < last + 1; i++) {
        ...
        if (i === first || i === last) {
            a.attr({
                id: i === first ? "prev-button" : "next-button",
                href: "#",
                ariaLabel: i === first ? "Previous" : "Next"
            }).text(i === first ? "Prev" : "Next");
        } else {
...
function reloadCategory(reloadType) {
    ...
    if (reloadType === 1) {
        ...
        getCategoryOrderBy(sortColumn, sortType, currentPage, pageSize).then(response => {
            init(parseJSON(response));
        }).finally(() => {
            setTimeout(() => {
                document.getElementById('preloader-table').style.opacity = '0';
...
function parseJSON(response) {
    if (response["lstAuditLog"] && response["lstAuditLog"].length > 0) {
        response["lstAuditLog"] = response["lstAuditLog"].map(item => JSON.parse(item));
    }
    return response;
}

// Helper function to fetch list of category
async function getAllCategory(page, pageSize) {
    const response = await interact(`/api/log-management/audit-log?page=${page}&pageSize=${pageSize}`);
    if (response) {
        return response;
    }
}
...
async function getSingleCategory(id) {
    const response = await interact(`/api/log-management/audit-log/${id}`);
    if (response) {
        return response;
    }
}
...
async function getCategoryOrderBy(sortColumn, sortType, page, pageSize) {
    const response = await interact(`/api/log-management/audit-log?sortColumn=${sortColumn}&sortType=${sortType}&page=${page}&pageSize=${pageSize}`);
    if (response) {
        return response;
    }
}
...
async function deleteCategories(ids) {
    const response = await interact(`/api/log-management/audit-log/delete?ids=${ids}`, {
        method: 'DELETE'
    })
    if (response) {
        return response;
...
async function insertOrUpdateCategory(categoryObject, type) {
    ...
    const response = await interact(`/api/log-management/audit-log/${url}`, {
        method,
        headers: {
