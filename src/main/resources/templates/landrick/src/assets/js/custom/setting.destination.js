...
function createCategoryDOM(response) {
    ...
    for (let i = 0; i < response["lstConfig"].length; i++) {
        ...
        for (let j = 0; j < 7; j++) {
            if (j === 0) {
                ...
                const checkBox = $("<input>").attr({
                    type: "checkbox",
                    "data-id": response['lstConfig'][i]['id'],
                    "data-status": response['lstConfig'][i]['isActivated']
                }).addClass("form-check-input border-dark");
                ...
                if (keys[j] === "isActivated") {
                    ...
                    div.addClass(`badge ${response["lstConfig"][i][keys[j]] ? 'bg-soft-success' : 'bg-soft-danger'} rounded px-3 py-1`)
                        .text(`${response["lstConfig"][i][keys[j]] ? 'Active' : 'Inactive'}`);
                    ...
                    // Set title based on key
                    div.addClass(`badge ${response["lstConfig"][i][keys[j]] ? 'bg-soft-success' : 'bg-soft-danger'} rounded px-3 py-1`)
                        .text(`${response["lstConfig"][i][keys[j]] ? 'Conneced' : 'Not Connected'}`);
                    ...
                    const editButton = $("<button></button>")
                        ...
                        .attr({
                            ...
                            onclick: `${response["lstConfig"][i]["editButton"]["callback"]}(${response["lstConfig"][i]["id"]})`
                        })
                        .html(response["lstConfig"][i]["editButton"]["icon"])
                    ...
                    const checkButton = $("<button></button>")
                        ...
                        .attr({
                            type: "button",
                            id: "checkBtn",
                            onclick: `${response["lstConfig"][i]["checkButton"]["callback"]}(${response["lstConfig"][i]["id"]})`
                        })
                        .html(response["lstConfig"][i]["checkButton"]["icon"])
                    // Add button to table
                    div.append(editButton);
                    div.append(checkButton);
                    td.append(div);
                } else {
                    td.text(response["lstConfig"][i][keys[j]]);
                }
                tableBodyRow.append(td);
            }
        }
...
function parseJSON(response) {
    if (response["lstConfig"] && response["lstConfig"].length > 0) {
        response["lstConfig"] = response["lstConfig"].map(item => JSON.parse(item));
    }
